...
import { EntityProps, Faction, GameObjectProps } from '../types.ts';
...
describe('findShortestPath', () => {
  ...
  beforeEach(() => {
    ...
    const blockProps: GameObjectProps = {
      type: 'entity',
      x: 0,
      y: 1,
    ...
    entity = new Entity(entityProps);

    const targetGameObjectProps: GameObjectProps = {
      type: 'gameObject',
    ...
    new GameObject(targetGameObjectProps, itemFactory);
  });
  ...
  test('go around the wall even if there is a entity on path, but ignore gameObject in target coordinates', () => {
    ...
    const blockingEntityProps: EntityProps = {
      speed: 1,
      type: 'entity',
      x: 1,
      y: 1,
    ...
    new Entity(blockingEntityProps);
