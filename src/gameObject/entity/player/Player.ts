...
class Player extends Entity {
  ...
  private takeInteraction(direction?: Direction) {
    ...
    const interactiveGameObjects = gameObjectsFromField.filter((gameObject) =>
      gameObject.getIsInteractive()
    );
  ...
  private addMoveListener() {
        ...
        switch (key) {
          case 'w':
            if (this.isInteracting) {
              emitPlayerMakeTurn(this, () =>
                this.takeInteraction(Direction.UP)
              );
              return;
            }
          ...
          case 's':
            if (this.isInteracting) {
              emitPlayerMakeTurn(this, () =>
                this.takeInteraction(Direction.DOWN)
              );
              return;
            }
            if (this.isUsingItemTouchable) {
              emitPlayerMakeTurn(this, () =>
                this.useItem(Direction.DOWN, this)
              );
              return;
            }
            this.resetInteraction();
          ...
          case 'a':
            if (this.isInteracting) {
              emitPlayerMakeTurn(this, () =>
                this.takeInteraction(Direction.LEFT)
              );
              return;
            }
            if (this.isUsingItemTouchable) {
              emitPlayerMakeTurn(this, () =>
                this.useItem(Direction.LEFT, this)
              );
              return;
            }
            this.resetInteraction();
          ...
          case 'd':
            if (this.isInteracting) {
              emitPlayerMakeTurn(this, () =>
                this.takeInteraction(Direction.RIGHT)
              );
              return;
            }
            if (this.isUsingItemTouchable) {
              emitPlayerMakeTurn(this, () =>
                this.useItem(Direction.RIGHT, this)
              );
              return;
            }
            this.resetInteraction();
    ...
    document.addEventListener('click', (event: PointerEvent) => {
      if (this.isUsingItemProjectable) {
        const getCoordinatesFromCanvasClick = (
        ...
        if ((event.target as HTMLElement).id === 'canvas') {
          const coordinatesFromCanvasClick: Coordinates =
            getCoordinatesFromCanvasClick(event.target as HTMLCanvasElement);
          const playerAndCenterDifference: Coordinates =
            GameState.getPlayerAndCenterDifference();
          const fieldSize: number = GameState.getFieldSize();

          const targetX =
            Math.floor(coordinatesFromCanvasClick.x / fieldSize) +
            playerAndCenterDifference.x;
          const targetY =
            Math.floor(coordinatesFromCanvasClick.y / fieldSize) +
            playerAndCenterDifference.y;

          emitPlayerMakeTurn(this, () =>
            this.useItem({ x: this.x, y: this.y }, { x: targetX, y: targetY })
          );
        }
      }
    });
